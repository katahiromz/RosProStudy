...
MsgCrackに含まれているwinwords.txtは、サクラエディタの入力補完に使うことができる。
サクラエディタの「設定」→「タイプ別設定」→「支援」の「入力補完」でwinwords.txtのある場所を指定する。
...
今度はダイアログアプリを作ってみよう。dialog.cとして次のソースを入力する。
...
リソースデータはUTF-8に対応しているので、簡単に日本語を埋め込むことができる。

【演習7】RisohEditorを使ってdialog_res.rcに比較的大きなボタンを追加せよ。
そのボタンのキャプションは「押してね」とし、ボタンのIDは「psh1」とする。

【演習8】OnCommandを編集してpsh1を押したらメッセージボックスを表示するようにせよ。
ただしMessageBoxの第一引数にダイアログのHWNDを指定すること。
