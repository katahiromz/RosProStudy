...
EDITTEXTはクラス名EDITのテキストボックス。edt1, edt2, edt3はコントロールID。
...
テキストボックスの中身が変更されたとき、OnCommandにEN_CHANGEが届く。
これならテキストボックスを変更した直後に結果がすぐ表示できる。
