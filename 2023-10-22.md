...
Gitで変更したファイルのリストを表示するには「git status」を実行する。

変更した内容を出力するには「git diff」を実行する。
変更内容は差分（difference）とかパッチ（patch）などと呼ばれる。
「git diff > my-file.diff」などとすると変更内容をファイル「my-file.diff」に出力できる。
...
「git apply my-file.diff」などとすると、差分ファイル「my-file.diff」を適用できる。
