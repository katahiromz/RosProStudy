...
一応やり方をさらりと説明すると、「ファイル」メニューで新しくソリューションを作成し、ソリューションにコンパイルすべきソースファイルを追加する。
「Debug」か「Release」を選ぶ。そして「ビルド」するだけだ。具体的な細かい点はVisual Studioの種類によって異なり、ここで紹介するようなものではない。

プロジェクトの種類を説明すると、ハッカーが使うような黒い画面が出て来るようなアプリは「コンソールアプリ」という。
黒い画面が表示されず、代わりに通常のウィンドウが表示されるのが「ウィンドウアプリ」「Win32アプリ」である。
C/C++を使う場合は、「Visual C++」が必要になる。
...
CMakeでは、CMakeLists.txtというテキストファイルでビルド方法を指定する。RosBEにはCMakeが内蔵されている。
...
GUI版のCMakeをインストール。CMakeをやり直すのでCMakeCache.txtを削除。
